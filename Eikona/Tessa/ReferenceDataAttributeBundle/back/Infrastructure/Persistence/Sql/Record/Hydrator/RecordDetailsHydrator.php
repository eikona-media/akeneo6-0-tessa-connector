 ...
 */
namespace Eikona\Tessa\ReferenceDataAttributeBundle\back\Infrastructure\Persistence\Sql\Record\Hydrator;
 ...
 * @see \Akeneo\ReferenceEntity\Infrastructure\Persistence\Sql\Record\Hydrator\RecordDetailsHydrator
 */
class RecordDetailsHydrator implements RecordDetailsHydratorInterface
{
    ...
    private function getImage($valueCollection, $attributeAsImage): Image
    {
        ...
        if (!empty($imageValue)) {
            ...
            if ($this->isTessaImageData($imageValue['data'])) {
                $assetId = current($imageValue['data']);

                $asset = $this->tessa->getAsset($assetId);
                if (is_object($asset) && $asset->data && $asset->data->id) {
                    $file = new FileInfo();
                    $file->setKey('tessa-' . $asset->data->id);
                    $file->setOriginalFilename($asset->data->file_name);
                } else {
                    return $result;
                }
            } else {
